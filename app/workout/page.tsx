...
function getExerciseType(ex: Exercise | undefined): ExerciseType {
  return ex?.type ?? "weighted";
}

function formatSetValue(weightKg: number, reps: number, type: ExerciseType): string {
  if (type === "bodyweight") return `自重 × ${reps}回`;
  if (type === "timed")      return `${reps}秒`;
  return `${weightKg}kg × ${reps}`;
}

function formatRestPreset(p: number): string {
  if (p < 60) return `${p}秒`;
...
function WorkoutInner() {
  ...
  const exerciseType = useMemo(
    (): ExerciseType => getExerciseType(selectedEx),
    [selectedEx],
  );
  ...
  const weightChips = useMemo<WeightChip[]>(() => {
    ...
    return chips;
  }, [lastSuggestion, bestWeights, selectedExerciseId, exercises, exerciseType]);
  ...
  const submitSet = async (weightKg: number, reps: number, feeling?: string) => {
    ...
    if (exerciseType === "weighted" && weightKg <= 0) return;
    setIsSaving(true);
    setError(null);
    setInfo(null);

    // PR check only for weighted exercises
    const currentOneRm = exerciseType === "weighted" ? estimateOneRepMax(weightKg, reps) : 0;
    const prevBestOneRm = bestOneRms[selectedExerciseId];
    const isNewPr = exerciseType === "weighted" &&
      (prevBestOneRm === undefined ? true : currentOneRm > prevBestOneRm);
  ...
  const handleAddSet = () => {
    const wKg = exerciseType === "weighted" ? currentWeight : 0;
    submitSet(wKg, currentReps);
  };

  const handleQuickSame = () => {
    if (!lastSuggestion) return;
    const wKg = exerciseType === "weighted" ? lastSuggestion.weightKg : 0;
    submitSet(wKg, lastSuggestion.reps);
  };

  const handleQuickPlus = () => {
    if (!lastSuggestion) return;
    if (exerciseType === "bodyweight") {
      submitSet(0, lastSuggestion.reps + 1);
    } else if (exerciseType === "timed") {
      submitSet(0, lastSuggestion.reps + 10);
    } else {
      submitSet(Math.round((lastSuggestion.weightKg + effectiveStep) * 10) / 10, lastSuggestion.reps);
    }
  };
  ...
  const handleSaveEdit   = () => {
    ...
    const editEx = exercises.find((e) => e.id === editingSet?.exerciseId);
    const editType = getExerciseType(editEx);
    if (editType === "weighted" && editingWeight <= 0) return;
    const weightToSave = editType === "weighted" ? editingWeight : 0;
    setSets((prev) => prev.map((s) => s.id === editingSetId ? { ...s, weightKg: weightToSave, reps: editingReps } : s));
  ...
  const quickSameLabel = (() => {
    if (!lastSuggestion) return "";
    if (exerciseType === "bodyweight") return `自重 × ${lastSuggestion.reps}回`;
    if (exerciseType === "timed")      return `${lastSuggestion.reps}秒`;
    return `${lastSuggestion.weightKg}kg × ${lastSuggestion.reps}回`;
  })();

  const quickPlusLabel = (() => {
    if (!lastSuggestion) return "";
    if (exerciseType === "bodyweight") return `＋1回  →  自重 × ${lastSuggestion.reps + 1}回`;
    if (exerciseType === "timed")      return `＋10秒  →  ${lastSuggestion.reps + 10}秒`;
    const newW = Math.round((lastSuggestion.weightKg + effectiveStep) * 10) / 10;
    return `＋${effectiveStep}kg  →  ${newW}kg × ${lastSuggestion.reps}回`;
  })();

  const lastSuggestionLabel = (() => {
    if (!lastSuggestion || lastSuggestion.exerciseId !== selectedExerciseId) return "";
    if (exerciseType === "bodyweight") return `前回 自重 × ${lastSuggestion.reps}回`;
    if (exerciseType === "timed")      return `前回 ${lastSuggestion.reps}秒`;
    return `前回 ${lastSuggestion.weightKg}kg × ${lastSuggestion.reps}回`;
  })();
    ...
    <div className="min-h-screen">
      ...
      <div className="flex items-center gap-3 px-5 pt-10 pb-4">
        ...
        <div className="flex-1">
          ...
          {lastSuggestionLabel && (
            <p className="text-[11px] text-stone">{lastSuggestionLabel}</p>
          )}
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="ml-1 flex shrink-0 items-center gap-1">
                    {exType === "bodyweight" && (
                      <span className="text-[9px] text-stone opacity-70">自重</span>
                    )}
                    {exType === "timed" && (
                      <span className="text-[9px] text-stone opacity-70">秒</span>
                    )}
                    {count > 0 && (
          ...
          >
            ...
            {lastSuggestion && !showCustomInput ? (
              <div className="space-y-2">
                ...
                >
                  <span className="text-terracotta">{quickPlusLabel}</span>
                </button>
              ...
              <div className="space-y-4">
                ...
                {/* Weight steppers row (weighted only) */}
                ...
                {/* Bodyweight: only reps stepper */}
                {exerciseType === "bodyweight" && (
                  <div>
                    <div className="flex items-center gap-2 rounded-xl bg-ivory px-3 py-2 mb-3">
                      <span className="text-sm">🏋️</span>
                      <span className="text-[12px] font-medium text-stone">自重トレーニング</span>
                    </div>
                    <p className="text-[10px] font-semibold text-stone uppercase tracking-[0.5px] mb-2">回数</p>
                    <div className="flex items-center gap-1 rounded-xl bg-ivory p-1">
                      <button
                ...
                {/* Timed: seconds stepper */}
                {exerciseType === "timed" && (
                  <div>
                    <div className="flex items-center gap-2 rounded-xl bg-ivory px-3 py-2 mb-3">
                      <span className="text-sm">⏱️</span>
                      <span className="text-[12px] font-medium text-stone">秒数で記録</span>
                    </div>
                    <p className="text-[10px] font-semibold text-stone uppercase tracking-[0.5px] mb-2">秒数</p>
                    <div className="flex items-center gap-1 rounded-xl bg-ivory p-1">
                      <button
                        type="button"
                        onClick={() => setCurrentReps((v) => Math.max(5, v - 10))}
                        className="h-9 w-9 rounded-lg bg-card flex items-center justify-center active:scale-[0.95] transition"
                        style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}
                      >
                        <Minus size={14} strokeWidth={2.5} className="stroke-charcoal" />
                    ...
                    <p className="text-[10px] text-pale text-center mt-1">秒</p>
                  </div>
                )}

                {/* 増減幅 toggle (weighted only) */}
                {exerciseType === "weighted" && (
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] text-stone">増減幅</span>
                ...
                {/* Weight chips (weighted only) */}
                {exerciseType === "weighted" && weightChips.length > 0 && (
                  <WeightInput
                    value={currentWeight}
                ...
                <button
                  ...
                  disabled={
                    !selectedExerciseId ||
                    (exerciseType === "weighted" && currentWeight <= 0) ||
                    currentReps <= 0 ||
                    isSaving
                  }
                ...
                >
                  {isSaving
                    ? "保存中…"
                    : exerciseType === "bodyweight"
                    ? `自重 × ${currentReps}回 を記録`
                    : exerciseType === "timed"
                    ? `${currentReps}秒 を記録`
                    : `${currentWeight}kg × ${currentReps} を記録`}
                </button>
              </div>
            )}
          </div>
        ...
        <div className="px-5 mb-4">
          ...
          <div className="space-y-4">
            {groupedSets.map(([exerciseId, groupSets]) => {
              ...
              const exType = getExerciseType(ex);
              const statLabel = exType === "bodyweight"
                ? `${groupSets.reduce((s, x) => s + x.reps, 0)}回`
                : exType === "timed"
                ? `${groupSets.reduce((s, x) => s + x.reps, 0)}秒`
                : `${groupSets.reduce((s, x) => s + x.weightKg * x.reps, 0).toLocaleString()}kg`;
              return (
                <div key={exerciseId}>
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-[13px] font-bold text-charcoal">{exName}</span>
                    <span className="text-[11px] text-stone">
                      {groupSets.length}セット · {statLabel}
                    </span>
                  </div>
                          ...
                          <div className="flex-1">
                            {isEditing ? (
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] text-stone">
                                  {exType === "weighted" ? "×" : exType === "timed" ? "秒" : "回"}
                                </span>
                                <input
                                  type="number"
                                  inputMode="numeric"
                              ...
                              <span className="text-[15px] font-bold text-charcoal tracking-[-0.3px]">
                                {formatSetValue(set.weightKg, set.reps, exType)}
                              </span>
                            )}
                          </div>
